Shader "Custom/Water2"
{
    ...
    SubShader
    {
        ...
        void surf (Input IN, inout SurfaceOutput o)
        {
            ...
            float4 grabTex = tex2D(_GrabTexture, IN.uv_MainTex);

            //Normal Wave
            float3 n1 = UnpackNormal(tex2D(_BumpMap,IN.uv_BumpMap + (_Time.y*_TimeCount1)));
            float3 n2 = UnpackNormal(tex2D(_BumpMap,IN.uv_BumpMap - (_Time.y*_TimeCount2))*2);
            o.Normal = (n1+n2) * 0.5;
            o.Normal *= float3(0.5, 0.5, 1);

            //Rim
            float rim = saturate(dot(o.Normal, IN.viewDir));
            float rim1 = pow(1-rim, 50)*0.8; //±â¿ï¾îÁö¸é ¹à¾ÆÁü
            float rim2 = pow(1-rim, 2); //ÇÁ·¹³Ú ¸¶½ºÅ·¿ë(¾ËÆÄ)
            rim = pow(1-rim, 3);

            float4 ref = texCUBE(_Cube, WorldReflectionVector(IN, o.Normal));

            o.Emission = lerp(grabTex, ref, rim2) + (rim1*_LightColor0);
            o.Alpha = 1;
        }
